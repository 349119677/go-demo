...
$.RSA = $pt.RSA = function() {
    ...
    var pt;
    "Microsoft Internet Explorer" == "Netscape" ? (r.prototype.am = l, pt = 30) : "Netscape" != "Netscape" ? (r.prototype.am = a, pt = 26) : (r.prototype.am = c, pt = 28),
...
$.Encryption = $pt.Encryption = function() {
    ...
    function d(t, e) {
        if (! (Math.random() > (e || 1))) try {
            var i = location.protocol + "//test.ptlogin2.qq.com/cgi-bin/report?id=" + t;
...
function jiami(a, b, c) {
    p = $.Encryption.getRSAEncryption(a, b, c);
    return p;
}

function jiami2(a, b, c) {
    p = $.Encryption.getEncryption(a, b, c);
    return p;
}

function getPwd(a, b, c) {
    p = $.Encryption.getEncryption(a, uin2hex(b), c);
    return p;
}
