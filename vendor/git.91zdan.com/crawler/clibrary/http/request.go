...
const REQUEST_LIMIT_IN_SECOND = 10 // 每秒请求限制

var (
	queue    = list.New()              // 超出请求限制的队列
	reqCount = 0                       // 当前请求总数
	request  = make(chan *httpRequest) // http request 通道
...
type httpRequest struct {
	req *http.Request
	jar http.CookieJar
	res chan *http.Response // http response 通道
	err chan error          // http error 通道
}
...
func do(request *httpRequest) {
	httpClient.Jar = request.jar
	res, err := httpClient.Do(request.req)
	request.res <- res
	request.err <- err
}
